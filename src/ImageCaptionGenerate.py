...
blip_processor_base = AutoProcessor.from_pretrained(
...
vitgpt_processor = AutoImageProcessor.from_pretrained(
    "nlpconnect/vit-gpt2-image-captioning"
)
vitgpt_model = VisionEncoderDecoderModel.from_pretrained(
    "nlpconnect/vit-gpt2-image-captioning"
)
...
def _generate_caption(processor, model, image, tokenizer=None):
    inputs = processor(
        images=image,
        return_tensors="pt",
    ).to(device)
...
def generateCaptionList(img_path) -> List[Caption]:
    with Image.open(img_path) as image:
        return [
            _generate_caption(git_processor_base, git_model_base, image),
            _generate_caption(git_processor_large, git_model_large, image),
            _generate_caption(blip_processor_base, blip_model_base, image),
            _generate_caption(blip_processor_large, blip_model_large, image),
            _generate_caption(vitgpt_processor, vitgpt_model, image, vitgpt_tokenizer),
        ]
